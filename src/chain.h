...
public:
    ...
    uint256 nAccumulatorCheckpoint{0};
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action)
    {
        ...
        if (nSerVersion >= DBI_SER_VERSION_NO_ZC) {
            // Serialization with CLIENT_VERSION = 1009902+
            READWRITE(nFlags);
            ...
            // Serialization with CLIENT_VERSION = 1009901
            ...
            // Serialization with CLIENT_VERSION = 1009900-
            int64_t nMint = 0;
            uint256 hashNext{};
            int64_t nMoneySupply = 0;
            READWRITE(nMint);
