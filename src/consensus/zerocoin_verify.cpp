...
bool RecalculateYEPSupply(int nHeightStart, bool fSkipZyep)
{
    ...
    while (true) {
        ...
        if (!fSkipZyep && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
            UpdateZYEPSupplyConnect(block, pindex, true);
        }
