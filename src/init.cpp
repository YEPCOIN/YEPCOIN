...
#include "zyep/accumulatorcheckpoints.h"
#include "zyepchain.h"
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZyepThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "yep.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "yepd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the YEP and zYEP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 16740, 16742));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in YEP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in YEP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, yep, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in YEP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all YEP specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-yepstake=<n>", strprintf(_("Enable or disable staking functionality for YEP inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zyepstake=<n>", strprintf(_("Enable or disable staking functionality for zYEP inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzyep=<n>", strprintf(_("Enable automatic wallet backups triggered after each zYEP minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zyepbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zYEP backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 16741, 16743));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The YEP Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that YEP is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. YEP is shutting down."));
...
#endif
    // Make sure only a single YEP process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. YEP is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //YEP: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //YEP specific: zerocoin and spork DB's
                ...
                // YEP: load previous sessions sporks if we have them.
                ...
                //     CAmount zyepSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                //     if (pblockindex->GetZerocoinSupply() < zyepSupplyCheckpoint) {
                //         // Trigger reindex due wrapping serials
                //         LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zyepSupplyCheckpoint/COIN);
                //         reindexDueWrappedSerials = true;
                //     } else if (pblockindex->GetZerocoinSupply() > zyepSupplyCheckpoint) {
                //         // Trigger global zYEP reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateYEPSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
                ...
                //     CAmount zyepSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                //     if (pblockindex->GetZerocoinSupply() != zyepSupplyCheckpoint)
                //         return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zyepSupplyCheckpoint/COIN));
                // }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of YEP") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart YEP to complete") << "\n";
        ...
        zwalletMain = new CzYEPWallet(pwalletMain->strWalletFile);
        ...
        if (zwalletMain->GetMasterSeed() != 0) {
            //Inititalize zYEPWallet
            uiInterface.InitMessage(_("Syncing zYEP wallet..."));

            pwalletMain->InitAutoConvertAddresses();

            bool fEnableZYEPBackups = GetBoolArg("-backupzyep", true);
            pwalletMain->setZYEPAutoBackups(fEnableZYEPBackups);

            //Load zerocoin mint hashes to memory
            pwalletMain->zyepTracker->Init();
    ...
    /* Denominations
       ...
       For example:
       1YEP+1000 == (.1YEP+100)*10
       10YEP+10000 == (1YEP+1000)*10
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZyepThread(threadGroup);
    }
