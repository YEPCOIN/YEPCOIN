...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zYEP mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight >= 2 && !isPoSActive) {
        nSubsidy = 100 * COIN;

        if (nHeight == 330) {
            nSubsidy = 1000000 * COIN;
        }
    } else if (isPoSActive && nHeight <= 22601) {
        nSubsidy = 80 * COIN;
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!UpdateZYEPSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zYEP supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("yep-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZYEPSupplyConnect(block, pindex, fJustCheck)) {
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateYEPSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nYepBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nYepBadBlockBits)) {
...
int ActiveProtocol()
{
    ...
    // SPORK_15 was used for 70918 (v1.0.0), commented out now.
