...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zyeps that have been in mempool long
                    //and higher priority to zyeps that are large in value
                    ...
                    // zYEP spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zYEP spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zYEP serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        LogPrint("yep", "CreateNewBlock(): total size %u\n", nBlockSize);
        ...
        if (pblock->nVersion < 7) {
            if (fZerocoinActive) {
                ...
                if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                    //For the period before v2 activation, zYEP will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zYEP with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }

                //Sign block with the zYEP key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zYEP key failed \n", __func__);
                    return NULL;
                }
        ...
        if (!TestBlockValidity(state, *pblock, pindexPrev, false, false)) {
            LogPrint("yep", "CreateNewBlock() : TestBlockValidity failed\n");
            mempool.clear();
            return NULL;
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("YEPMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->zyepTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->zyepTracker->ListMints(true, true, true); //update the state
        }
        return error("YEPMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("YEPMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yep-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("yep", "Running YEPMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("YEPMiner exception");
    } catch (...) {
        LogPrintf("YEPMiner exception");
    }

    LogPrintf("YEPMiner exiting\n");
}
