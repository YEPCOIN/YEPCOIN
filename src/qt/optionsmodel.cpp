...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::YEP);
    ...
    if (settings.contains("nAnonymizeYEPAmount") || reset)
        SoftSetArg("-anonymizeyepamount", settings.value("nAnonymizeYEPAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in yep.cpp during shotdown to also remove the window positions
