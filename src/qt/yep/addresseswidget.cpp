...
#include "qt/yep/forms/ui_addresseswidget.h"
#include "qt/yep/addresslabelrow.h"
#include "qt/yep/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(YEPGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter YEP address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress yepAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(yepAdd.Get(), label.toUtf8().constData(),
                yepAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
