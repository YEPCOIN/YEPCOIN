...
#include "qt/yep/dashboardwidget.h"
#include "qt/yep/forms/ui_dashboardwidget.h"
#include "qt/yep/sendconfirmdialog.h"
#include "qt/yep/txrow.h"
...
DashboardWidget::DashboardWidget(YEPGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of YEP and zYEP staked."));
    ...
    ui->labelAmountZyep->setText("0 zYEP");
    ui->labelAmountYEP->setText("0 YEP");
    setCssProperty(ui->labelAmountYEP, "text-stake-yep-disable");
    setCssProperty(ui->labelAmountZyep, "text-stake-zyep-disable");
    ...
    ui->btnHowTo->setText(tr("How to get YEP or zYEP"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("YEP")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isYEP = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZYEP;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair YEP, zYEP
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            yep = (pair.first != 0) ? pair.first / 100000000 : 0;
            zyep = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalYEP += pair.first;
            chartData->totalZyep += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesYEP.append(yep);
        chartData->valueszYEP.append(zyep);

        int max = std::max(yep, zyep);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("YEP");
    set1 = new QBarSet("zYEP");
    set0->setColor(QColor(30,30,30));
    ...
    set0->append(chartData->valuesYEP);
    set1->append(chartData->valueszYEP);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalYEP > 0 || chartData->totalZyep > 0) {
        setCssProperty(ui->labelAmountYEP, "text-stake-yep");
        setCssProperty(ui->labelAmountZyep, "text-stake-zyep");
    } else {
        setCssProperty(ui->labelAmountYEP, "text-stake-yep-disable");
        setCssProperty(ui->labelAmountZyep, "text-stake-zyep-disable");
    }
    forceUpdateStyle({ui->labelAmountYEP, ui->labelAmountZyep});
    ui->labelAmountYEP->setText(GUIUtil::formatBalance(chartData->totalYEP, nDisplayUnit));
    ui->labelAmountZyep->setText(GUIUtil::formatBalance(chartData->totalZyep, nDisplayUnit, true));
