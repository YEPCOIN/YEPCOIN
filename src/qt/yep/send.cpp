...
#include "qt/yep/send.h"
#include "qt/yep/forms/ui_send.h"
#include "qt/yep/addnewcontactdialog.h"
#include "qt/yep/qtutils.h"
#include "qt/yep/sendchangeaddressdialog.h"
#include "qt/yep/optionbutton.h"
#include "qt/yep/sendconfirmdialog.h"
#include "qt/yep/myaddressrow.h"
#include "qt/yep/guitransactionsutils.h"
...
#include "zyepcontroldialog.h"

SendWidget::SendWidget(YEPGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("YEP");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zYEP");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (YEP) or private coins (zYEP)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("YEP address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 YEP");
    ...
    // setCssProperty(coinIcon, "coin-icon-yep");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onYEPSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onYEPSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZyep = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZyep));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZyep ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendYEP = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendYEP ? AskPassphraseDialog::Context::Send_YEP : AskPassphraseDialog::Context::Send_zYEP, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendYEP) ? send(recipients) : sendZyep(recipients)) {
...
bool SendWidget::sendZyep(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zYEP is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zYEP selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZYEPControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZYEPControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZyep(
        ...
        inform(tr("zYEP transaction sent!"));
        ZYEPControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZYEP_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zYEP require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zYEP transaction
        ...
        emit message("zYEP transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isYEP){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any YEP to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZYEPControlDialog *zYEPControl = new ZYEPControlDialog(this);
            zYEPControl->setModel(walletModel);
            zYEPControl->exec();
            ui->btnCoinControl->setActive(!ZYEPControlDialog::setSelectedMints.empty());
            zYEPControl->deleteLater();
        } else {
            inform(tr("You don't have any zYEP in your balance to select."));
        }
    }
}
...
void SendWidget::onYEPSelected(bool _isYEP){
    isYEP = _isYEP;
    setCssProperty(coinIcon, _isYEP ? "coin-icon-yep" : "coin-icon-zyep");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress yepAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(yepAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(yepAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
