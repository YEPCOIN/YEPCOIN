...
#include "qt/yep/forms/ui_snackbar.h"
#include "qt/yep/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(YEPGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &YEPGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
