...
#include "ui_zyepcontroldialog.h"
...
std::set<std::string> ZYEPControlDialog::setSelectedMints;
std::set<CMintMeta> ZYEPControlDialog::setMints;

bool CZYEPControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZYEPControlDialog::COLUMN_DENOMINATION || column == ZYEPControlDialog::COLUMN_VERSION || column == ZYEPControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZYEPControlDialog::ZYEPControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZYEPControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zYEP Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZYEP->setProperty("cssClass", "text-main-purple");
    ui->labelZYEP_int->setProperty("cssClass", "text-main-purple");
...
ZYEPControlDialog::~ZYEPControlDialog()
{
    delete ui;
}

void ZYEPControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZYEPControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZYEPControlWidgetItem* itemDenom(new CZYEPControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZYEPControlWidgetItem *itemMint = new CZYEPControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zYEP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zYEP seed used to mint this zYEP is not the same as currently hold in the wallet";
            else
...
void ZYEPControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZYEPControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZYEP_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZYEPControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZYEPControlDialog::GetSelectedMints()
{
...
void ZYEPControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
