...
CZYEPStake::CZYEPStake(const libzerocoin::CoinSpend& spend)
{
...
int CZYEPStake::GetChecksumHeightFromMint()
{
...
int CZYEPStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZYEPStake::GetChecksum()
{
    return nChecksum;
}

// The zYEP block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZYEPStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZYEPStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zYEP staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZYEPStake::GetUniqueness()
{
    //The unique identifier for a zYEP is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZYEPStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZYEPStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zYEP that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZYEPOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zYEP output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zYEP", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZYEPOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zYEP output", __func__);
...
bool CZYEPStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZYEPStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzYEPTracker* zyepTracker = pwallet->zyepTracker.get();
    CMintMeta meta;
    if (!zyepTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zyepTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!YEP Stake
bool CYEPStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CYEPStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CYEPStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CYEPStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CYEPStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CYEPStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CYEPStake::GetUniqueness()
{
    //The unique identifier for a YEP stake is the outpoint
...
CBlockIndex* CYEPStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
