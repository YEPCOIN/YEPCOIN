...
bool CYepStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CYepStake from zerocoin spend", __func__);
...
bool CYepStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CYepStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CYepStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CYepStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CYepStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CYepStake::GetUniqueness() const
{
...
bool CYepStake::ContextCheck(int nHeight, uint32_t nTime)
{
