...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "yep" is a composite category enabling all YEP-related debug output
            if (ptrCategory->count(std::string("yep"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\YEP
// Windows >= Vista: C:\Users\Username\AppData\Roaming\YEP
// Mac: ~/Library/Application Support/YEP
// Unix: ~/.yep
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YEP";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "yep.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty yep.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override yep.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "yepd.pid"));
