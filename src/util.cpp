...
const char * const YEP_CONF_FILENAME = "yep.conf";
const char * const YEP_PID_FILENAME = "yep.pid";
const char * const YEP_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YEPParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "YEPParams";
#else
    // Unix
    return pathRet / ".yep-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", YEP_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", YEP_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", YEP_PID_FILENAME));
