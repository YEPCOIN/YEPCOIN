...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current YEP address for receiving payments to this account.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"yepaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"yepaddress\"  (string, required) The yep address to be associated with an account.\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    const bool fMintChange = params[1].get_bool();       // Mint change to zYEP
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST) {
        if (fMintChange)
            throw JSONRPCError(RPC_WALLET_ERROR, "zYEP minting is DISABLED (except for regtest), cannot mint change");

        if (!isPublicSpend)
            throw JSONRPCError(RPC_WALLET_ERROR, "zYEP old spend only available in regtest for tests purposes");
    }

    std::vector<CZerocoinMint> vMintsSelected;
    return DoZyepSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, isPublicSpend);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZyepSpend(nAmount, false, true, vMintsSelected, address_str, isPublicSpend);
}


extern UniValue DoZyepSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend)
{
...
UniValue setzyepseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw std::runtime_error(
            "setzyepseed \"seed\"\n"
...
UniValue getzyepseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw std::runtime_error(
            "getzyepseed\n"
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    ...
    CzYEPWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzyepstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                "dzyepstate\n"
...
UniValue searchdzyep(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw std::runtime_error(
            "searchdzyep\n"
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) YEP address for auto minting\n" +
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZyepSpend(mint.GetDenominationAsAmount(), false, true, vMintsSelected, address_str, isPublicSpend);
}
