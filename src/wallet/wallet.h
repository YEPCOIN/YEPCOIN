...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 20000 YEP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zYEP send
enum ZerocoinSpendStatus {
    ZYEP_SPEND_OKAY = 0,                            // No error
    ZYEP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZYEP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZYEP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZYEP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZYEP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZYEP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZYEP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZYEP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZYEP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZYEP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZYEP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZYEP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZYEP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZYEP_SPENT_USED_ZYEP = 14,                      // Coin has already been spend
    ZYEP_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZYEP_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZYEPBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZYEPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzyepAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzYEPTracker> zyepTracker;
    ...
    void setZWallet(CzYEPWallet* zwallet);
    CzYEPWallet* getZWallet();
    bool isZeromintEnabled();
    void setZYEPAutoBackups(bool fEnabled);
    ...
    /// Get 20000 YEP output and keys which can be used for the Masternode
    ...
    /** zYEP reset */
    boost::signals2::signal<void()> NotifyzYEPReset;
