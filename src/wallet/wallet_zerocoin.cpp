    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZYEPSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZYEPSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zyepseed to wallet";
    }
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZYEPSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZYEPSeed(hashSeed, vchSeed)) {
...
void CWallet::doZYepRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZYEP)
{
    //If this is a zapwallettx, need to read zpiv
    if (fCheckZYEP && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
...
bool CWallet::CreateZYEPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwallet->GenerateDeterministicZYEP(denomination, coin, dMint);
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    receipt.SetStatus("Spend Successful", ZYEP_SPEND_OKAY);  // When we reach this point spending zPIV was successful
...
void CWallet::setZWallet(CzYEPWallet* zwallet)
{
    ...
    zyepTracker = std::unique_ptr<CzYEPTracker>(new CzYEPTracker(this));
}
