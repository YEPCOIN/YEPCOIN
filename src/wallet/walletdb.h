...
public:
    ...
    bool WriteZYEPSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZYEPSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZYEPSeed_deprecated(uint256& seed);
    bool EraseZYEPSeed();
    bool EraseZYEPSeed_deprecated();

    bool WriteZYEPCount(const uint32_t& nCount);
    bool ReadZYEPCount(uint32_t& nCount);
