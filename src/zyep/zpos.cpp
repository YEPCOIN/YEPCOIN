...
bool CLegacyZYepStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZYepStake from non zc-spend", __func__);
    ...
    *this = CLegacyZYepStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zyep stake origin", __func__);

    // All good
    return true;
}

CLegacyZYepStake::CLegacyZYepStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZYepStake::GetIndexFrom()
{
...
CAmount CLegacyZYepStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZYepStake::GetUniqueness() const
{
...
bool CLegacyZYepStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zYEP stake block: height %d outside range", __func__, nHeight);
