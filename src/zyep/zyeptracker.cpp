...
CzYEPTracker::CzYEPTracker(CWallet* parent)
{
    this->wallet = parent;
...
{
    bool iszYEPWalletInitialized = (nullptr != zYEPWallet);
    CMintMeta meta;
    ...
    if (!iszYEPWalletInitialized)
        zYEPWallet = new CzYEPWallet(wallet);
...
std::set<CMintMeta> CzYEPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzyep from DB\n", __func__, listDeterministicDB.size());
    }
