...
CzYEPTracker::CzYEPTracker(std::string strWalletFile)
{
...
CzYEPTracker::~CzYEPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzYEPTracker::Init()
{
...
bool CzYEPTracker::Archive(CMintMeta& meta)
{
...
bool CzYEPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzYEPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzYEPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzYEPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzYEPTracker::GetSerialHashes()
{
...
CAmount CzYEPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzYEPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzYEPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzYEPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzYEPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzYEPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzYEPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzYEPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzYEPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzYEPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzYEPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzYEPWallet* zYEPWallet)
{
    bool iszYEPWalletInitialized = (NULL != zYEPWallet);
    CMintMeta meta;
    ...
    if (! iszYEPWalletInitialized)
        zYEPWallet = new CzYEPWallet(strWalletFile);
    meta.isSeedCorrect = zYEPWallet->CheckSeed(dMint);
    if (! iszYEPWalletInitialized)
        delete zYEPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzYEPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzYEPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzYEPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzYEPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzYEPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzYEPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzYEPWallet* zYEPWallet = new CzYEPWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zYEPWallet);
        }
        delete zYEPWallet;
        LogPrint("zero", "%s: added %d dzyep from DB\n", __func__, listDeterministicDB.size());
    }
