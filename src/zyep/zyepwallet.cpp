        ...
        if (walletdb.ReadZYEPSeed_deprecated(seed)) {
            ...
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZYEPSeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zyep seed\n", __func__);
                }
            }
        }
    }
...
bool CzYEPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    else if (!walletdb.ReadZYEPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzYEPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
void CzYEPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzYEPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzYEPWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzYEPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZYEP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
uint512 CzYEPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzYEPWallet::GenerateDeterministicZYEP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzYEPWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzYEPWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzYEPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
