...
{
    CDataStream serializedCoinSpend = ZYEPModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZYEPSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
...
bool UpdateZYEPSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
