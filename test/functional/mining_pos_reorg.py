...
class ReorgStakeTest(YepTestFramework):
    ...
    def check_money_supply(self, expected_yep, expected_zyep):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected YEP and zYEP supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_yep))
            for denom in node['zYEPsupply']:
                assert_equal(node['zYEPsupply'][denom], DecimalAmt(expected_zyep[denom]))
    ...
    def run_test(self):
        ...
        # Check YEP and zYEP supply at the beginning
        # ------------------------------------------
        # zYEP supply: 2 coins for each denomination
        expected_zyep_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # YEP supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zyep_supply)
        ...
        # Verify that YEP and zYEP supplies were properly updated after the spends and reorgs
        self.log.info("Check YEP and zYEP supply...")
        ...
        expected_zyep_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zyep_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zyep_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zyep_supply)
