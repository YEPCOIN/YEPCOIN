...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/YEPParams"
    else
        PARAMS_DIR="$HOME/.yep-params"
    fi
fi
...
Failed to fetch the YEP zkSNARK parameters!
...
This script will fetch the YEP zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common YEP zkSNARK parameters. Note that it is
